...
begin
  ...
  begin
    ...
    if RadioGroup1.ItemIndex <> 6 then
    begin
      if (ADOSPConsulta.fieldbyname('código').AsString <> '') and (ADOSPConsulta.RecordCount > 0) then begin
    ...
    if RadioGroup1.ItemIndex <> 6 then
    begin
      if (ADOSPConsulta.fieldbyname('código').AsString <> '') and (ADOSPConsulta.RecordCount > 0) then
      begin
        Sql.Add('and I.cdProduto = :CDPRODUTO1 ');
        Parameters.ParamByName('CDPRODUTO1').Value := ADOSPConsulta.fieldbyname('Código').AsString;
      end;
    end;
    if NOT (FrmCancelamentoVenda.Possui_Permissao('854','V',FrmPrincipalPreVenda.cbxUsuario.Text,FrmPrincipalPreVenda.EdtUsuario.Text,false)) then begin
    ...
    FrmRelEntSai.QRLblSelecao.Caption := 'Produto-> '+ADOSPConsulta.fieldbyname('Descrição').AsString;
    FrmRelEntSai.QRLblTitulo.Caption  := 'Saídas';
...
begin
  ...
  begin
    ...
    vIdent := InputBox('Localizar também pela especificação', 'Entre com o texto para pesquisa, ou pressione enter para cotinuar!', '');
    ...
    begin
      LblEspecificacao.Caption := 'Localizar especificação contendo: ';
