...
uses
  ...
  FormRelLivrosFaltamChegar, FormConsultaEscolas, MoPreVenda, System.ImageList;
Type
...
uses uFuncoesPadrao, DataModulo, RelOrcamentoLista, FrmPrincipal,
  FormRelAlunosLivros;
...
begin
//  FormPrincipal.Lista1.enabled := true;
...
//                'isnull(IP.nrQtd - Ip.nrQtdChegada,0) as faltaChegar                          '+
                ...
                'left join PedidoCompra PC on IP.nrLancto = PC.nrLancto                       '+
...
begin
  ...
  case RgTipoRel.ItemIndex of
    0: begin
      ...
      LblPeriodo.Caption := 'Período dos Pedidos';
    end;
    1: begin
      cbxProdutoFiltro.Enabled := false;
      dtInicial.Date := Strtodate('01/11/2015');
...
var qry: TADOQuery;
begin
  if (EdtSequencial.Text <> '') and (EdtPrevenda.Text <> '') then begin
    if nrSequencialAnterior <> EdtSequencial.Text then begin
...
//      if EdtCdPessoa.Text <> '' then begin
//        sql.Text := 'select case when (PE.Existir = ''F'') then PF.dsNaturalidade      '+
//                    '   when (PE.Existir = ''J'') then PJ.nmContato                    '+
//                    'End as Apelido                                                    '+
//                    'from Pessoa PE left join P_Fisica PF on PF.cdPessoa = PE.cdPessoa '+
//                    'left join P_Juridica PJ on PJ.cdPessoa = Pe.cdPessoa              '+
//                    'where PE.cdPessoa = :cdPessoa ';
//        parameters.ParamByName('cdPessoa').Value := EdtCdPessoa.Text;
//        open;
