...
//{$IFDEF PREVENDA}
uses uFuncoesPadrao, DataModulo, MoPreVenda;
//{$ELSE}
//uses FrmPrincipal, uFuncoesPadrao, DataModulo, MoPreVenda;
//{$ENDIF}
...
begin
  inherited;
  try
    if MessageDlg('Confirma a exclusão desta Escola?', MtConfirmation,
    ...
    With ADOQryExcluir do begin
      ...
      try       // parametros para a query de exclusao
        ...
        DModulo.Conexao.RollbackTrans;
        MessageDlg('Você não pode excluir esta escola!',MtWarning,[mbOk],0);
      end;
    end;
    atualizaQryConsulta;
    LimpaTodosCampos(TbSheetMdCadastroDados);
    ...
    on E:Exception do
    begin
      ShowMessage('Não foi possivel excluir esta escola! Motivo ->'+E.message);
    end;
  end;
 end;
